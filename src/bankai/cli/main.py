...
def _identify_via_tvdb(query: str, *, kind: MediaKind) -> MediaIdentity | None:
    ...
    if not results:
        console.print(
            f"[warn]TheTVDB returned no matches for[/warn] [accent]{query}[/accent] [muted]\u2014 continuing without metadata.[/muted]"
        )
...
def _menu_run_movie() -> None:
    ...
    console.print(
        f"[success]queued[/success] job [accent]{job.id}[/accent] \u2014 \u2018Queue / history\u2019 to watch / cancel."
    )
...
def _menu_run_show() -> None:
    ...
    if result is None:
        console.print(
            f"[warn]no episodes found for[/warn] [accent]{show}[/accent] S{season:02d} [muted](tried {site_choice if site_id else 'all sites'})[/muted]"
        )
        return
    ...
    for ep in selected:
        ...
        console.print(
            f"[success]queued[/success] {q} \u2014 job [accent]{job.id}[/accent] [muted]({result.site})[/muted]"
        )
...
def _render_background_progress(job: Any) -> None:
    ...
    console.print(
        f"[bold]Step:[/bold] [magenta]{snapshot.step_label}[/magenta]  [bold]Overall:[/bold] {_progress_text(snapshot.overall_percent)}"
    )
...
def shell() -> None:
    ...
    while True:
        ...
        if line == "help":
            console.print(
                "Commands: search QUERY, run QUERY [URL], shows SHOW SEASON, queue, config, doctor, exit"
            )
            continue
        try:
...
def extract_audio(
    ...
    want_video: bool = typer.Option(
        False, "--want-video", help="Download video too (for visual sync)."
    ),
    max_height: int | None = typer.Option(
        None, "--max-height", help="Cap video height when --want-video."
    ),
    as_json: bool = typer.Option(
        False, "--json", help="Print a single JSON result line on stdout."
    ),
) -> None:
...
def web_install_service(
    ...
    if not no_enable:
        console.print(
            f"[green]started[/green] {SERVICE_NAME} \u2014 open [accent]http://{settings.web.host}:{settings.web.port}[/accent]"
        )
...
def review_replace_torrent(
    ...
    async def go() -> dict[str, Any] | None:
        ...
        result = await _run_worker_once(
            TorrentWorker(), get_settings().paths.work_dir, payload=payload
        )
...
@app.command("anime-download", hidden=True)
def anime_download(
    release_title: str = typer.Option(..., "--release-title"),
    torrent_url: str = typer.Option(..., "--torrent-url"),
    detail_url: str = typer.Option(..., "--detail-url"),
    ...
    async def go() -> None:
        try:
            ...
            reason = f"{type(exc).__name__}: {exc}"
            log.debug("anime download traceback", exc_info=True)
            log.error("Anime download failed — %s", reason)
            print(reason, flush=True)
            raise typer.Exit(code=1) from exc
