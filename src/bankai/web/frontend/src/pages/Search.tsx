    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Find a {kind} on TheTVDB, then match it on filmpalast.
        </p>
      </header>
      ...
      {!configured ? (
        <EmptyState
          ...
          description="Add your TVDB API key in Settings to search."
        />
      ) : loading ? (
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {selected?.name}
              {german && german !== selected?.name && (
                <Badge variant="accent">DE: {german}</Badge>
              )}
            </DialogTitle>
            <DialogDescription>
              {loadingFilm
          ...
          {loadingFilm ? (
            ...
            <EmptyState
              icon={SearchIcon}
              title="No filmpalast match"
              description="This title may not be available on filmpalast."
            />
          ) : (
